...
export default function DataInputForm({ onSubmit, isLoading }: DataInputFormProps) {
  ...
  function handleSubmit(values: z.infer<typeof formSchema>) {
    const submissionValues = {
      ...values,
      historicalFinancials: values.historicalFinancials?.filter(item => item.year) // Filter out empty/uninitialized items
    };
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
            <Accordion type="multiple" defaultValue={accordionDefaultValues} className="w-full">
              ...
              <AccordionItem value="item-2">
                ...
                <AccordionContent className="pt-4 space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                    {/* ARR, GrossMargin, NewBizGrowth, ExpansionGrowth, Churn, NRR */}
                    <FormField control={form.control} name="arr" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Annual Recurring Revenue (ARR) *</FormLabel> <div className="relative"> <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 1000000" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Total current ARR in USD.</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="grossMargin" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Gross Margin *</FormLabel> <div className="relative"> <BadgePercent className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" step="0.01" placeholder="e.g., 0.8 for 80%" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Current gross profit margin (decimal).</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="newBusinessARRGrowthRate" render={({ field }) => ( <FormItem> <FormLabel className="text-base">New Business ARR Growth Rate *</FormLabel> <div className="relative"> <TrendingUpIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" step="0.01" placeholder="e.g., 0.2 for 20%" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Annual growth from new customers (decimal).</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="expansionARRGrowthRate" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Expansion ARR Growth Rate *</FormLabel> <div className="relative"> <Handshake className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" step="0.01" placeholder="e.g., 0.1 for 10%" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Annual growth from existing customers (decimal).</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="churnRate" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Annual Churn Rate *</FormLabel> <div className="relative"> <TrendingDownIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" step="0.01" placeholder="e.g., 0.1 for 10%" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Annual customer/revenue churn rate (decimal).</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="netRevenueRetention" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Net Revenue Retention (NRR) *</FormLabel> <div className="relative"> <FileText className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" step="0.01" placeholder="e.g., 1.1 for 110%" {...field} className="pl-10" /></FormControl> </div> <FormDescription>NRR or DBNER (decimal, e.g., 1.1 for 110%).</FormDescription> <FormMessage /> </FormItem> )}/>
                  </div>
                </AccordionContent>
              </AccordionItem>
              ...
              <AccordionItem value="item-3">
                ...
                <AccordionContent className="pt-4 space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                    {/* COGS, S&M Spend %, R&D Spend %, G&A Spend %, EBITDA */}
                    <FormField control={form.control} name="costOfGoodsSold" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Cost of Goods Sold (COGS) <span className="text-xs text-muted-foreground">(Optional)</span></FormLabel> <div className="relative"> <FactoryIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 250000" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Annual COGS in USD.</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="salesMarketingSpendPercentage" render={({ field }) => ( <FormItem> <FormLabel className="text-base">S&M Spend (% ARR) *</FormLabel> <div className="relative"> <Percent className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" step="0.01" placeholder="e.g., 0.4 for 40%" {...field} className="pl-10" /></FormControl> </div> <FormDescription>S&M spend as % of ARR (decimal).</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="researchDevelopmentSpendPercentage" render={({ field }) => ( <FormItem> <FormLabel className="text-base">R&D Spend (% ARR) *</FormLabel> <div className="relative"> <Cpu className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" step="0.01" placeholder="e.g., 0.2 for 20%" {...field} className="pl-10" /></FormControl> </div> <FormDescription>R&D spend as % of ARR (decimal).</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="generalAdministrativeSpendPercentage" render={({ field }) => ( <FormItem> <FormLabel className="text-base">G&A Spend (% ARR) <span className="text-xs text-muted-foreground">(Optional)</span></FormLabel> <div className="relative"> <Library className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" step="0.01" placeholder="e.g., 0.15 for 15%" {...field} className="pl-10" /></FormControl> </div> <FormDescription>G&A spend as % of ARR (decimal).</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="ebitda" render={({ field }) => ( <FormItem> <FormLabel className="text-base">EBITDA <span className="text-xs text-muted-foreground">(Optional)</span></FormLabel> <div className="relative"> <LineChart className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 0 or -50000" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Annual EBITDA in USD. Can be negative.</FormDescription> <FormMessage /> </FormItem> )}/>
                  </div>
                </AccordionContent>
              </AccordionItem>
              ...
              <AccordionItem value="item-4">
                ...
                <AccordionContent className="pt-4 space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                    {/* CAC, LTV/CAC Ratio, Total Customers, CAC by Channel (Text), Cohort Summary (Text) */}
                    <FormField control={form.control} name="customerAcquisitionCost" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Customer Acquisition Cost (CAC) *</FormLabel> <div className="relative"> <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 5000" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Average CAC in USD.</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="ltvToCacRatio" render={({ field }) => ( <FormItem> <FormLabel className="text-base">LTV to CAC Ratio *</FormLabel> <div className="relative"> <Users className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" step="0.1" placeholder="e.g., 3.5" {...field} className="pl-10" /></FormControl> </div> <FormDescription>LTV to CAC ratio (e.g., 3 for 3:1).</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="totalCustomers" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Total Active Customers <span className="text-xs text-muted-foreground">(Optional)</span></FormLabel> <div className="relative"> <UsersRound className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 150" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Total number of current active customers.</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="cacByChannel" render={({ field }) => ( <FormItem className="md:col-span-2"> <FormLabel className="text-base">CAC by Channel <span className="text-xs text-muted-foreground">(Optional)</span></FormLabel> <div className="relative"> <Route className="absolute left-3 top-3 h-5 w-5 text-muted-foreground" /> <FormControl><Textarea placeholder="e.g., Google Ads: $600, SEO: $300, Referrals: $100" {...field} className="pl-10 min-h-[80px]" /></FormControl> </div> <FormDescription>Approx. CAC for key acquisition channels.</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="cohortAnalysisSummary" render={({ field }) => ( <FormItem className="md:col-span-2"> <FormLabel className="text-base">Cohort Analysis Summary <span className="text-xs text-muted-foreground">(Optional)</span></FormLabel> <div className="relative"> <PieChart className="absolute left-3 top-3 h-5 w-5 text-muted-foreground" /> <FormControl><Textarea placeholder="e.g., 12-month net dollar retention for Q1 2023 cohort: 110%. LTV for enterprise segment: $50k." {...field} className="pl-10 min-h-[100px]" /></FormControl> </div> <FormDescription>Brief summary of cohort retention trends and LTV insights.</FormDescription> <FormMessage /> </FormItem> )}/>
                  </div>
                </AccordionContent>
              </AccordionItem>
              ...
              <AccordionItem value="item-pricing-sales">
                ...
                <AccordionContent className="pt-4 space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                    <FormField control={form.control} name="pricingTiers" render={({ field }) => ( <FormItem className="md:col-span-2"> <FormLabel className="text-base">Pricing Tiers</FormLabel> <div className="relative"> <Tags className="absolute left-3 top-3 h-5 w-5 text-muted-foreground" /> <FormControl><Textarea placeholder="e.g., Basic: $29/mo, Pro: $149/mo, Enterprise: Custom" {...field} className="pl-10 min-h-[80px]" /></FormControl> </div> <FormDescription>Describe your main pricing tiers/packages.</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="averageDealSize" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Average Deal Size (ACV)</FormLabel> <div className="relative"> <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 12000" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Average Annual Contract Value in USD.</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="averageContractLengthMonths" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Avg. Contract Length (Months)</FormLabel> <div className="relative"> <CalendarDays className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 12" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Average customer contract length in months.</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="salesCycleLengthDays" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Avg. Sales Cycle (Days)</FormLabel> <div className="relative"> <CalendarClock className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 60" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Average sales cycle length in days.</FormDescription> <FormMessage /> </FormItem> )}/>
                  </div>
                   <FormField control={form.control} name="customerAcquisitionChannels" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Main Customer Acquisition Channels</FormLabel> <div className="relative"> <Megaphone className="absolute left-3 top-3 h-5 w-5 text-muted-foreground" /> <FormControl><Textarea placeholder="e.g., SEO (40%), Google Ads (30%), Content Marketing (20%), Referrals (10%)" {...field} className="pl-10 min-h-[80px]" /></FormControl> </div> <FormDescription>List key channels and their approximate contribution if known.</FormDescription> <FormMessage /> </FormItem> )}/>
                  <FormField control={form.control} name="marketingSpendBreakdown" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Marketing Spend Breakdown</FormLabel> <div className="relative"> <PieChart className="absolute left-3 top-3 h-5 w-5 text-muted-foreground" /> <FormControl><Textarea placeholder="e.g., Google Ads: $10k/month, Content Creation: $5k/month" {...field} className="pl-10 min-h-[80px]" /></FormControl> </div> <FormDescription>Optional breakdown of marketing spend by channel or category.</FormDescription> <FormMessage /> </FormItem> )}/>
                </AccordionContent>
              </AccordionItem>
              ...
              <AccordionItem value="item-team-capital">
                ...
                <AccordionContent className="pt-4 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-8">
                    <FormField control={form.control} name="totalEmployees" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Total Employees</FormLabel> <div className="relative"> <Users className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 50" {...field} className="pl-10" /></FormControl> </div> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="salesTeamSize" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Sales Team Size</FormLabel> <div className="relative"> <Users2 className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 10" {...field} className="pl-10" /></FormControl> </div> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="marketingTeamSize" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Marketing Team Size</FormLabel> <div className="relative"> <Megaphone className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 5" {...field} className="pl-10" /></FormControl> </div> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="engineeringTeamSize" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Engineering Team Size</FormLabel> <div className="relative"> <Cpu className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 20" {...field} className="pl-10" /></FormControl> </div> <FormMessage /> </FormItem> )}/>
                  </div>
                  <h3 className="text-lg font-medium text-foreground/90 mt-4">Capital Efficiency</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                    <FormField control={form.control} name="cashBurnRateMonthly" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Monthly Cash Burn/Gain</FormLabel> <div className="relative"> <Gauge className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., -10000 or 5000" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Avg. monthly net cash burn (negative) or gain (positive) in USD.</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="cashRunwayMonths" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Cash Runway (Months)</FormLabel> <div className="relative"> <CalendarClock className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 18" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Estimated months of cash runway remaining.</FormDescription> <FormMessage /> </FormItem> )}/>
                  </div>
                  <h3 className="text-lg font-medium text-foreground/90 mt-4">Debt & Equity</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                     <FormField control={form.control} name="totalDebt" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Total Outstanding Debt</FormLabel> <div className="relative"> <Banknote className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 50000" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Total outstanding debt in USD.</FormDescription> <FormMessage /> </FormItem> )}/>
                     <FormField control={form.control} name="equityStructureSummary" render={({ field }) => ( <FormItem className="md:col-span-2"> <FormLabel className="text-base">Equity Structure Summary</FormLabel> <div className="relative"> <Library className="absolute left-3 top-3 h-5 w-5 text-muted-foreground" /> <FormControl><Textarea placeholder="e.g., Common stock only, or Series A ($5M raised, 20% ownership, 1x non-participating preference)" {...field} className="pl-10 min-h-[80px]" /></FormControl> </div> <FormDescription>Brief summary of equity structure (e.g., funding rounds, preferences).</FormDescription> <FormMessage /> </FormItem> )}/>
                  </div>
                  <h3 className="text-lg font-medium text-foreground/90 mt-4">Product Usage</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                    <FormField control={form.control} name="dailyActiveUsers" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Daily Active Users (DAU)</FormLabel> <div className="relative"> <AppWindow className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 1000" {...field} className="pl-10" /></FormControl> </div> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="monthlyActiveUsers" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Monthly Active Users (MAU)</FormLabel> <div className="relative"> <AppWindow className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="number" placeholder="e.g., 5000" {...field} className="pl-10" /></FormControl> </div> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="keyFeatureAdoptionRate" render={({ field }) => ( <FormItem className="md:col-span-2"> <FormLabel className="text-base">Key Feature Adoption</FormLabel> <div className="relative"> <Target className="absolute left-3 top-3 h-5 w-5 text-muted-foreground" /> <FormControl><Textarea placeholder="e.g., Dashboard Usage: 80% of MAU, Reporting Feature: 40% of MAU" {...field} className="pl-10 min-h-[80px]" /></FormControl> </div> <FormDescription>Adoption rate of 1-2 key product features.</FormDescription> <FormMessage /> </FormItem> )}/>
                  </div>
                </AccordionContent>
              </AccordionItem>
              ...
              <AccordionItem value="item-context">
                ...
                <AccordionContent className="pt-4 space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                    <FormField control={form.control} name="fundingStage" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Funding Stage</FormLabel> <div className="relative"> <LandmarkIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="text" placeholder="e.g., Series A, Bootstrap" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Company's current funding stage.</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="industryVertical" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Industry Vertical</FormLabel> <div className="relative"> <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="text" placeholder="e.g., FinTech, HealthTech" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Primary industry your company operates in.</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="targetMarket" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Target Market</FormLabel> <div className="relative"> <Users2 className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="text" placeholder="e.g., SMB, Mid-Market, Enterprise" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Primary customer segment.</FormDescription> <FormMessage /> </FormItem> )}/>
                    <FormField control={form.control} name="customerGeographicConcentration" render={({ field }) => ( <FormItem> <FormLabel className="text-base">Geographic Concentration</FormLabel> <div className="relative"> <Globe className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> <FormControl><Input type="text" placeholder="e.g., North America: 70%, Europe: 20%" {...field} className="pl-10" /></FormControl> </div> <FormDescription>Primary geographic markets and revenue contribution.</FormDescription> <FormMessage /> </FormItem> )}/>
                  </div>
                </AccordionContent>
              </AccordionItem>
            </Accordion>
